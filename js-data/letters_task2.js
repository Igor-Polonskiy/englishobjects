(() => {
    ...
    let task = document.querySelector('.leters_task2')
    ...
    let bugzone = task.querySelector('.bug')
    let flowerzone = task.querySelector('.flower')
    let bugs = bugzone.querySelectorAll('img')
    let flowers = flowerzone.querySelectorAll('img')
    let pot = task.querySelector('.pot')
    let dropField
    ...
    task.addEventListener('pointerdown', (e) => {
        if ((e.target.classList.contains('letters_task_dragitem') || e.target.classList.contains('letters_task_dragitem_dropped'))&&mistake < 4) {
            mouseDown(e)
        }
    });
    ...
    function resetTask() {
        ...
        dragZone.innerHTML = ''
        filldragZone()
        bugs.forEach(item => item.classList.add('nodisplay'))
        pot.classList.remove('nodisplay')
        flowers.forEach(item => item.classList.add('nodisplay'))
        mistake = 0
    }
    ...
    function checkAnswer() {
        ...
        if (draggingItem.innerText === dropField.getAttribute('data-letter')) {
            ...
            flowers[flowers.length - dataOrder].classList.remove('nodisplay')


        } else {
            draggingItem.style.background = 'red'
            mistake++
            if (mistake < 4) {
                bugs[mistake - 1].classList.remove('nodisplay')
                if (mistake > 1 && mistake < 4) {
                    bugs[mistake - 2].classList.add('nodisplay')
                }
            }else{
                bugs[mistake - 1].classList.remove('nodisplay')
                bugs[mistake - 2].classList.add('nodisplay')
                pot.classList.add('nodisplay')
            }
