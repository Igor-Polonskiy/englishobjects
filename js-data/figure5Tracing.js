(() => {
    ...
    let task = document.querySelector('.figure5_wrapper')
    ...
    function setupCanvas() {
        ...
        drawfigure2(5,140, 'rgb(230,230,230)');

        pixels = cx.getImageData(0, 0, c.width, c.height);
        figure2pixels = getpixelamount(230, 230, 230);
    }

    function drawfigure2(n, size, color) {

        //cx.fillStyle =`rgb(${r}, ${g}, ${b+1})`
       
        let reg = Math.PI / 180 * (180 - (n - 2) * 180 / n);//(n>=3)
        drawTriangle(n,size, reg, `rgb(${r}, ${g}, ${b + 1})`)
        drawTriangle(n,size-15, reg, color)
        drawTriangle(n,size-35, reg, `rgb(${r}, ${g}, ${b + 1})`)
        drawTriangle(n,size-50, reg, 'white')
    };

    function drawTriangle(n,size, reg,color) {
        let centerX = c.width / 2;
        let centerY = c.height / 2;
        /* cx.moveTo(centerX- width/2, 0 + height + top_padding);
         cx.beginPath()
         cx.lineTo(centerX, 0 + top_padding);
         cx.lineTo(centerX+ width/2, 0 + height + top_padding);
         cx.lineTo(centerX- width/2, 0 + height + top_padding);
         cx.fillStyle =color
         cx.fill()*/
        
        // var reg = Math.PI / 5 * 4; пятиконечная звезда
        //cx.save();
        cx.beginPath();
        for (let i = 0; i <= n; i++) {
            let x = Math.sin(i * reg);
            let y = Math.cos(i * reg);
            cx.lineTo(centerX + x * size, centerY + y * size);
        }
        cx.fillStyle = color
        cx.fill()
        cx.closePath();
    ...
    function pixelthreshold() {
        ...
        if (getpixelamount(r, g, b) / figure2pixels > 1.4) {
    ...
    function pulse() {
        ...
        let timerId1 = setInterval(() => {
            ...
            drawfigure2(5,size, `rgb(${r}, ${g}, ${b + 1})`);
        }, 40);
